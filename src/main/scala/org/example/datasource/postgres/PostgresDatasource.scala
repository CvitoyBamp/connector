...
class PostgresScan(connectionProperties: ConnectionProperties) extends Scan with Batch {
  ...
  override def planInputPartitions(): Array[InputPartition] = {
    ...
    (for (parts <- 0 to count by step) yield new PostgresPartition(connectionProperties.url, connectionProperties.user, connectionProperties.password, parts, step)).toArray
  }
